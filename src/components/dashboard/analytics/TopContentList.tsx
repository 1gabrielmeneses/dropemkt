            ...
            <CardContent className="space-y-4 flex-1 min-h-[200px]">
                {!hasData ? (
                    <MissingDataPlaceholder
                        message="No Content Analyzed Yet"
                        subMessage="Save competitor content to see performance analytics here."
                        icon={FileVideo}
                    />
                ) : (
                    contentList.slice(0, 5).map((item) => (
                        <div key={item.id} className="flex items-center gap-4 group">
                            {/* Thumbnail */}
                            {item.thumbnail_url ? (
                                <img src={item.thumbnail_url} alt={item.title || "Content"} className="h-16 w-16 rounded-lg object-cover flex-shrink-0 bg-muted" />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0 grid grid-cols-12 gap-4 items-center">
                                <div className="col-span-12 sm:col-span-6">
                                    <p className="text-sm font-medium truncate group-hover:text-primary transition-colors cursor-pointer" title={item.title || ""}>{item.title || "Untitled Content"}</p>
                                    <p className="text-xs text-muted-foreground">
                                        Published {item.published_at ? new Date(item.published_at).toLocaleDateString() : "Unknown date"}
                                    </p>
                                </div>
